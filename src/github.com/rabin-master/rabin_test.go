...
func Test_Rabin(t *testing.T) {
	...
	buff := makeSequence(10480)
	buff1 := []byte{5, 6, 7, 8}
	buff2 := []byte{9, 10, 11, 12}
	buff3 := []byte{13, 14, 15, 0}
	hash := New()
	...
	cmp1 := RabinFingerprintFixed(buff1)
	cmp2 := RabinFingerprintFixed(buff2)
	cmp3 := RabinFingerprintFixed(buff3)
	if sum != cmp {
