...
func buildNewMatrix(dataShards, totalShards int) (matrix, error) {

	vm, err := Newvandermonde(totalShards-dataShards, dataShards)
	if err != nil {
		return nil, err
	}
...
func New(dataShards, parityShards int, opts ...Option) (Encoder, error) {
	...
	default:
		r.m, err = buildNewMatrix(dataShards, r.totalShards)
	}
	if err != nil {
		return nil, err
	}
...
func (r *reedSolomon) reconstruct(shards [][]byte, dataOnly bool, required []bool) error {
	...
	//r.dataShards是个数，数据块的个数

	//收集的是有效块
	subShards := make([][]byte, r.dataShards)
	validIndices := make([]int, r.dataShards) //有效
	invalidIndices := make([]int, 0)          //无效索引
	subMatrixRow := 0
	...
	//这里的outputs与指定要修的数据块有关，如果没指定就修丢失的
	...
	//fmt.Println("matrixRows",matrixRows)
	...
	//fmt.Println("matrixRows",matrixRows)
	//fmt.Println("shards",shards[:r.dataShards])
