...
func HandleSend(m message.ReplicaMessage) {
	...
	pub, _ := blsMgr.DecPublicKey(m.OnePublicKey)
	sign, _ := blsMgr.DecSignature(m.OneSign)
	err := pub.Verify(m.Payload, sign)
	if err == nil {
		receivedFrag.InsertValueAndInt(idint, m.Payload, m.Source)
	}

	ids, _ := receivedFrag.GetAllValue(idint)
	if len(ids) == 27 {
		rlock.Lock()
		DecodeEnd = true
		log.Println("Enough data", len(ids))
		resultData = ErasureDecoding(idint, 15, 39)
		EndTime = time.Now()
