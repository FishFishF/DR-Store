...
func StartRBC(instanceid int, input []byte) {
	...
	votenum = 0
	EncodeFinish = false
	fingerprintEnough = false
	EndReady = false
	NewFGNUM = 0
	ReadyEnd = false
	readynum = 0

	arrayLength := 1024 * 1024 * 3
	...
	if id == 7 {
		...
		data, suc := ErasureEncoding(array, 3, 8)
		if !suc {
			log.Fatal("Fail to erasure coding the input when start ECRBC!")
		}

		MyFingerPrint = GF8Mod(data[id])
		log.Println(MyFingerPrint)
		...
		EndTime = time.Now()
		log.Println("Sender Time", EndTime.Sub(StartTime))
		sender.MACBroadcast(msgbyte, message.RBC)
	} else {
		StartTime = time.Now()
		data, suc := ErasureEncoding(array, 3, 7)
		if !suc {
			log.Fatal("Fail to erasure coding the input when start ECRBC!")
		}
		AllfingerPrint := make([][]byte, 7)
		...
		MyMTBranch = branches[id]
		MyMTIndex = idxresult[id]
		MyMtRoot = mRoot
		MyPayload = data[id]
		MyFingerPrint = GF8Mod(data[id])
		//这里还要生成Merkle Proof
		EndTime = time.Now()
		//log.Println(AllfingerPrint[1])
		log.Println("prepare time", EndTime.Sub(StartTime))
		StartTime = time.Now()
	}
...
func doTask(id int, wg *sync.WaitGroup, data []byte, AllFingerPrint [][]byte) {
	defer wg.Done()
	AllFingerPrint[id] = GF8Mod(data)
	// 执行任务
